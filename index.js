...
function updateUserStats(userId) {
  ...
  if (!stats.users[userId]) stats.users[userId] = { lastSeen: today, count: 0 };
...
const CANCEL_KB = [[
  Button.inline("❌ لغو", Buffer.from("cancel"), "danger")
]];

const DONE_KB = [[
  Button.inline("🔄 فایل دیگه", Buffer.from("another"), "primary")
]];

const BACK_KB = [[
  Button.inline("🔙 بازگشت", Buffer.from("back_start"), "secondary")
]];

const CONFIRM_KB = (originalName, newName) => [[
  Button.inline("✅ تایید", Buffer.from(`confirm_${newName}`), "success"),
  Button.inline("✏️ ویرایش", Buffer.from("edit"), "primary"),
  Button.inline("❌ لغو", Buffer.from("cancel"), "danger"),
]];

const CANCEL_PROCESS_KB = [[
  Button.inline("⛔ لغو پردازش", Buffer.from("cancel_process"), "danger")
]];
      ...
      if (text === "/stats") {
        ...
        await client.sendMessage(chatId, {
          message:
            ...
            `👥 کاربران امروز: <b>${stats.today}</b>\n` +
            `👥 کاربران هفته: <b>${stats.week}</b>\n` +
    ...
    if (text && !text.startsWith("/")) {
      ...
      if (!state || state.stage !== "awaiting_name") {
        const tip = await client.sendMessage(chatId, {
          ...
          buttons: [[Button.inline("📖 راهنما", Buffer.from("help"), "secondary")]],
        });
        ...
        try {
          ...
          await client.downloadMedia(origMsg.media, {
            ...
            progressCallback: async (downloaded, total) => {
              ...
              const speed = elapsed > 0 ? (Number(downloaded) - downloadedBytes) / elapsed : 0;
              downloadedBytes = Number(downloaded);
              
              const remaining = total > 0n && speed > 0 
                ? Math.ceil((Number(total) - Number(downloaded)) / speed)
                : 0;
              const eta = remaining > 0 ? `${Math.floor(remaining / 60)}:${String(Math.floor(remaining % 60)).padStart(2, '0')}` : '';

              await client.editMessage(chatId, {
                message: statusMsg.id,
                text: buildDownloadMsg(newFileName, pct, Number(total) || fileSize, speed, eta),
                parseMode: "html",
                buttons: CANCEL_PROCESS_KB,
              }).catch(() => {});
            },
          ...
          await client.sendFile(chatId, {
            ...
            progressCallback: async (progress) => {
              ...
              const speed = elapsed > 0 ? (progress * fileSize - uploadedBytes) / elapsed : 0;
              uploadedBytes = progress * fileSize;
              
              const remaining = speed > 0 ? Math.ceil((fileSize - progress * fileSize) / speed) : 0;
              const eta = remaining > 0 ? `${Math.floor(remaining / 60)}:${String(Math.floor(remaining % 60)).padStart(2, '0')}` : '';
