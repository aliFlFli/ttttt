...
const ADMIN_ID  = "155824019"; // همیشه string
...
function saveConfig() {
  try { fs.writeFileSync(CONFIG_FILE, JSON.stringify(botConfig, null, 2)); } catch {}کد کامل و اصلاح‌شده با دقت روی تشخیص ادمین، نرمال‌سازی آیدی و سیستم نقره‌ای/طلایی:
...
function getUser(id) {
  ...
  if (!users[key]) {
    users[key] = {
      ...
      premiumUntil: 0,
      premiumLimit: 0,
      premiumTier: null, // "silver" | "gold"
    };
    saveUsers();
  }
  // ادمین هیچ‌وقت pending یا banned نمی‌ماند
...
export async function startBot() {
  ...
  // مطمئن شو ادمین در دیتابیس approved است
  getUser(ADMIN_ID);
  ...
  client.addEventHandler(async (event) => {
    ...
    if ((u.status === "banned" ||کد کامل و اصلاح‌شده با دقت روی تشخیص ادمین، نرمال‌سازی آیدی و سیستم نقره‌ای/طلایی:
...
export async function startBot() {
  ...
  client.addEventHandler(async (event) => {
    ...
    if (text.startsWith("/start")) {
      ...
      // ادمین همیشه مستقیم وارد می‌شود
      if (isAdmin) {
        if (!u.lang) u.lang = "fa";
        u.status = "approved";
        saveUsers();
    ...
    // فقط approved و premium می‌توانند از ربات استفاده کنند
    ...
    if (text && !text.startsWith("/")) {
      ...
      setUserState(key, {
        ...
        items,
        confirmToken: token,
کد کامل و اصلاح‌شده با دقت روی تشخیص ادمین، نرمال‌سازی آیدی و سیستم نقره‌ای/طلایی:
