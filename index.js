...
let BOT_USERNAME = "Bot"; // will be set after getMe()
...
function loadConfig() {
  try {
    ...
    return {
      maxFileSize: 2 * 1024 * 1024 * 1024, // 2 GB hard limit
      normalDailyLimit: 5 * 1024 * 1024 * 1024, // 5 GB
    };
  }
}
function saveConfig() {
...
function loadStats() {
  try {
    ...
    return { total: 0, totalBytes: 0, today: 0, week: 0 };
  }
}
function saveStats() {
...
function getUser(id) {
  ...
  if (!users[key]) {
    users[key] = {
      lang: null,
      status: "pending",          // pending | approved | premium | banned
      firstSeen: Date.now(),
      ...
      referredBy: null,
      referrals: [],              // list of approved referred user ids
      premiumUntil: 0,
      premiumLimit: 0,            // daily limit while premium
    };
    saveUsers();
  }
  return users[key];
...
function checkAndApplyReferralReward(referrerId) {
  ...
  if (count >= 5) {
    ...
    saveUsers();
    return { tier: 5, days: 30, limitGB: 15 };
  }
  if (count >= 3) {
    ref.status = "premium";
    ...
    saveUsers();
    return { tier: 3, days: 15, limitGB: 10 };
  }
  return null;
}
...
export async function startBot() {
  ...
  client.addEventHandler(async (event) => {
      ...
      if (text.startsWith("/approve ")) {
        ...
        if (userId) {
          ...
          if (target.referredBy) {
            ...
            if (!ref.referrals.includes(String(userId))) {
              ref.referrals.push(String(userId));
              saveUsers();
              const reward = checkAndApplyReferralReward(target.referredBy);
              if (reward) {
                await client.sendMessage(target.referredBy, {
                  message: "🎉 Premium activated!\n" + reward.tier + " referrals → " + reward.limitGB + " GB / " + reward.days + " days",
                }).catch(() => {});
              }
            }
          }
    ...
    if (text.startsWith("/start")) {
      ...
      if (payload.startsWith("ref_") && u.status === "pending" && !u.referredBy) {
        ...
        if (refId && refId !== String(chatId) && users[refId]) {
          u.referredBy = refId;
          saveUsers();
        }
      }
    ...
    // ─── Only approved/premium can use the bot ────────────
  ...
  client.addEventHandler(async (event) => {
    ...
    if (data === "setlang_fa" || data === "setlang_en") {
      ...
      await client.sendMessage(chatId, {
        message: t("langSet", newLang),
      });

      // If still pending → send approval request
      if (u.status === "pending") {
        await client.sendMessage(chatId, {
          message: t("pending", newLang),
        });
    ...
    if (data === "profile") {
      ...
      if (isPremium(u)) {
        ...
        premiumLine = "⭐ " + leftDays + (lang === "en" ? " days left\n" : " روز باقی‌مانده\n");
      }
    ...
    // Admin approve / reject from inline buttons
